...
import by.mrk.kirleon.service.TreeSort;
...
public class TreeSortTest {
	...
	private final TreeSort sortingService = Mockito.spy(new TreeSort());
	...
	public void shouldNotCallInnerMethodsWhenGivenEmptyArray() {
		...
		// then
		verify(sortingService, never()).addNote(any(), any());
		verify(sortingService, never()).inorderTraversal(any(), any(), anyInt());
	...
	public void shouldCallInnerMethodsAtLeastFewTimesWhenGivenNonEmptyArray() {
		...
		// then
		verify(sortingService, atLeast(3)).addNote(any(), any());
		verify(sortingService, atLeast(3)).inorderTraversal(any(), any(), anyInt());
	}
